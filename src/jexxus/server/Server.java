...
public class Server {
  ...
  private final HashMap<String, ServerConnection> udpClients =
  ...
  public Server(ConnectionListener listener, int tcpPort, int udpPort, boolean useSSL) {
    ...
    try {
      ServerSocketFactory socketFactory =
          useSSL ? SSLServerSocketFactory.getDefault() : ServerSocketFactory.getDefault();
      ...
      System.err.println("There is already a server bound to port " + tcpPort
          + " on this computer.");
    ...
    if (udpPort != -1) {
      try {
        ...
        System.err.println("There was a problem starting the server's UDP socket on port "
            + udpPort);
